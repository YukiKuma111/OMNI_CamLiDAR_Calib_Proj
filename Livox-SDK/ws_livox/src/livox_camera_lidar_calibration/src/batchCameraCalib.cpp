...
int main(int argc, char **argv) {
	...
	while (getline(fin, filename) && filename.size() > 1) {
		...
		// cout << filename << endl;	// Commented by yuku
		...
		if (0 == findChessboardCorners(imageInput, board_size, image_points_buf)) {
			...
			// exit(1);	// Commented by yuku
			image_count = image_count - 1;	// Added by yuku
			continue;	// Added by yuku
		}
		else {
			Mat view_gray;
			...
			cout << filename << " can find chessboard corners.\n";	// Added by yuku
		}
	}
