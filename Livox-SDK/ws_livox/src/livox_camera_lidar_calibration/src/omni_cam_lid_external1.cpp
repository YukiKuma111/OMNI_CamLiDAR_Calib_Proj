...
int error_threshold, image_width, image_height;
vector<float> init;
...
public:
    ...
    bool operator()(const T *_q, const T *_t, T *residuals) const {
        ...
        T theta = atan2(p_c[1], p_c[0]);  // 方位角
        theta = -theta;  // 方位角
        T phi = asin(p_c[2] / sqrt(p_c[0] * p_c[0] + p_c[1] * p_c[1] + p_c[2] * p_c[2]));  // 仰角

        // 将方位角和仰角转换为图像的像素坐标
        Eigen::Matrix<T, 2, 1> p_2;
        p_2[0] = (theta + M_PI) / (2 * M_PI) * pd.w;
        p_2[1] = (M_PI / 2 - phi) / M_PI * pd.h;
...
void getParameters() {
    ...
    // added by yuku
    if (!ros::param::get("image_width", image_width)) {
        cout << "Can not get the value of image_width" << endl;
        exit(1);
    }
    // added by yuku
    if (!ros::param::get("image_height", image_height)) {
        cout << "Can not get the value of image_height" << endl;
        exit(1);
    }
    init.resize(12);
...
int main(int argc, char **argv) {
    ...
    // Modified by yuku
    getOmniData(lidar_path, photo_path, image_width, image_height, pData); // modified by ziyue
    ...
    // Modified by yuku, refered to https://blog.csdn.net/2301_78593267/article/details/135310726
    ...
    float error[2] = {0, 0};
    // Modified by yuku
    getUVOmniError(intrinsic_path, extrinsic_path, lidar_path, photo_path, error, error_threshold, image_width, image_height); 
